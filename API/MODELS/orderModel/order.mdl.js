...
const OrderModel = {
  ...
  async updateOrder(
    id_pedido,
    { estado, total, metodo_de_pago, fecha_entrega_estimada, direccion },
  ) {
    const query = `
            UPDATE pedido
  ...
  async deleteProductsFromOrder(id_pedido) {
    const query = `DELETE FROM order_detail WHERE id_pedido = ?`;
    const [result] = await pool.query(query, [id_pedido]);
