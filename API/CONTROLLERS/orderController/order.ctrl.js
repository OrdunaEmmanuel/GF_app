...
const OrderController = {
  ...
  async getOrdersWithUnits(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ error: "Error al obtener los pedidos con unidades" });
      console.log(error);
    }
  },
    ...
    try {
      ...
      if (!term || !id_usuario) {
        return res
          .status(400)
          .json({
            error: "Se requiere un término de búsqueda y el ID del usuario",
          });
      }
    ...
    try {
      ...
      if (!id_pedido || !id_producto || !cantidad) {
        return res
          .status(400)
          .json({
            error: "id_pedido, id_producto y cantidad son obligatorios",
          });
      }
      const producto = await ProductModel.getProductById(id_producto);
      if (!producto) {
      ...
      res
        .status(201)
        .json({ message: "Producto agregado al pedido", id_pedido });
    } catch (error) {
      res.status(500).json({ error: "Error al agregar producto al pedido" });
      console.log(error);
    }
  ...
  async editOrder(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Todos los campos del pedido son obligatorios" });
      }
      ...
      if (productos && productos.length > 0) {
        for (const product of productos) {
          ...
          await OrderModel.updateProductInOrder(
            id_pedido,
            id_producto,
            cantidad,
          );
  ...
  async deleteProductFromOrder(req, res) {
    try {
      ...
      if (!deletedProduct) {
        return res
          .status(404)
          .json({ error: "Producto no encontrado en este pedido" });
      }
      ...
      res
        .status(500)
        .json({ error: "Error al eliminar el producto del pedido" });
      console.log(error);
    }
  },
  ...
  async editProducts(req, res) {
    try {
      ...
      if (!productos || productos.length === 0) {
        return res
          .status(400)
          .json({ error: "Se requieren productos para actualizar" });
      }
      ...
      res
        .status(500)
        .json({ error: "Error al actualizar los productos del pedido" });
      console.log(error);
    }
  },
