...
exports.createCliente = async (req, res) => {
  try {
    ...
    if (clienteExistente) {
      return res
        .status(400)
        .json({ error: "El número de celular ya está registrado." });
    }
...
exports.loginUser = async (req, res) => {
  try {
    ...
    await pool.query("UPDATE usuario SET token = ? WHERE id_usuario = ?", [
      refreshToken,
      user.id_usuario,
    ]);
...
exports.logoutUser = async (req, res) => {
  try {
    const userId = req.user.userId;
    await pool.query("UPDATE usuario SET token = NULL WHERE id_usuario = ?", [
      userId,
    ]);
...
exports.getLocalidadById = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(
      "SELECT * FROM localidad WHERE id_localidad = ?",
      [id],
    );
    if (rows.length === 0)
...
exports.getRutaById = async (req, res) => {
  const id = req.params.id;
  try {
    const [rows] = await pool.query("SELECT * FROM ruta WHERE id_ruta = ?", [
      id,
    ]);
    if (rows.length === 0)
